...
{
    srand(time(0));
    vector_t * h_vector = alloc_vector(A_HYPER, A_DEPTH, A_ROWS, A_COLS, IDENTITY);
    vector_t * h_weights = alloc_vector(K_HYPER, K_DEPTH, K_ROWS, K_COLS, IDENTITY);
    vector_t * h_output = alloc_vector(K_HYPER, A_HYPER, A_ROWS - K_ROWS + 1, A_COLS - K_COLS + 1, ZEROES);
    vector_t * cpu_output = alloc_vector(K_HYPER, A_HYPER, A_ROWS - K_ROWS + 1, A_COLS - K_COLS + 1, ZEROES);
   ...
   print_vector(h_weights);
    ...
    for(int zz = 0; zz < h_output->size ; zz++) {
        printf("%.1f ", h_vector_out[zz]);
	}
	printf("\n");
